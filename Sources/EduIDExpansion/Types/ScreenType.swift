...
enum ScreenType: Int, CaseIterable {
    ...
    func configureNavigationItem(item: UINavigationItem, target: Any? = nil, action: Selector? = nil) {
        ...
        case .homeScreen:
            ...
            // just logo
        case .confirmScreen, .verifyLoginScreen, .createPincodefirstEntryScreen, .createPincodeSecondEntryScreen, .biometricApprovalScreen, .firstTimeDialogScreen, .eduIDCreatedScreen, .checkMailScreen, .enterPhoneScreen:
            addLogoTo(item: item)
            item.hidesBackButton = true
        default:
            addLogoTo(item: item)
